...
- The project is solely based on the uv package manager:
  - `curl -LsSf https://astral.sh/uv/install.sh | sh`
- Also it requires docker with an installed docker compose plugin
- The example dataset should be downloaded to `./data/raw`:
...
- Start a prediction demo streamlit app: `make start-demo`
...
- By default raw data is loaded from `./data/raw/{CATEGORY}`
...
- Prediction endpoint
  ...
  - Note: "params" and "pred_probas" gives the customer the possibility to generate insight on where in the image an anomaly is detected and with which probability. (See: demo)
...
- **Drift detection / Metrics:** Evidently
...
- The demo allows to select a category and send a random defective or good image to the prediction endpoint
- The predicted anomaly areas are visualized with a heatmap
...
- Building and starting the docker compose stacks for the first time takes a while
