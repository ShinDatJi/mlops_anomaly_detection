...
def _load_model(config_file, report_file, model_file, category):
    ...
    with mlflow.start_run() as run:
        ...
        mlflow.keras.log_model(model, name="model", save_exported_model=False, registered_model_name=category)

    run_id = run.info.run_id
    client = MlflowClient()
    model_versions = client.search_model_versions(f"name = '{category}' and run_id = '{run_id}'")
    if not model_versions:
        raise RuntimeError("No model version found for this run")
    version = model_versions[0].version
    client.set_registered_model_alias(category, "champion", version)
